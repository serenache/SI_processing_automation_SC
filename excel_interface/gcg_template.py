...
class gcg_graph():
    ...
    def fig(self):
        ...
        if self.size=='A4-Landscape':
            ...
            ax1.text(18.5, 13, 'ClienNo/ProjectN0', **hfont)
            ax1.text(18.5, 190, 'Project Name', **hfont)
            ax1.text(18.5, 195, 'Client Name', **hfont)
            t = ax1.text(250, 13, 'Draft', **hfont)
            bb = t.get_window_extent(fig.canvas.get_renderer()).transformed(plt.gca().transData.inverted())
            t.set_position((279.2-(bb.x1-bb.x0), 13))
            t = ax1.text(250, 195, '26/11/2021', **hfont)
            bb = t.get_window_extent(fig.canvas.get_renderer()).transformed(plt.gca().transData.inverted())
            ...
            ax1.text(110, 45, 'Client Name', **hfont)
            ax1.text(110, 38, 'Project Name', **hfont)
            ax1.text(110, 32, 'Main Caption', **hfont)
            ax1.text(110, 25, 'SubCaption', **hfont)
            ax1.text(257, 35, 'Figure XX', **hfont)
            ax2 = fig.add_axes((0.265, 0.116, 0.111, 0.111))
            ax2.imshow(plt.imread('gcg_logo.png'))
            ax2.set_axis_off()
            x0=0.14
            y0=0.30
            x1=0.92
            ...
            ax1.text(35.5, 9, 'ClientNo/ProjectN0', **hfont)
            ax1.text(35.5, 195.5, 'Project Name', **hfont)
            ax1.text(35.5, 199, 'Client Name', **hfont)

            t = ax1.text(250, 9, 'Draft', **hfont)
            bb = t.get_window_extent(fig.canvas.get_renderer()).transformed(plt.gca().transData.inverted())
            t.set_position((262-(bb.x1-bb.x0), 9))

            t = ax1.text(250, 199, '26/11/2021', **hfont)
            bb = t.get_window_extent(fig.canvas.get_renderer()).transformed(plt.gca().transData.inverted())
            ...
            x0 = 78
            ax1.text(x0, 31, 'Client Name', **hfont)
            ax1.text(x0, 27, 'Project Name', **hfont)
            ax1.text(x0, 22, 'Main Caption', **hfont)
            ax1.text(x0, 18, 'SubCaption', **hfont)

            ax1.text(232, 24, 'Figure XX', **hfont)
    ...
    def chart(self, layout, data_x, data_y, labels, styles, chart_set):
        ax = self.fig.add_subplot(layout)
        ...
        leg_box = self.fig.transFigure.inverted().transform(leg.get_window_extent())
        leg_dy = leg_box[1][1] - leg_box[0][1]
        leg_coord = ax.transAxes.inverted().transform(self.fig.transFigure.transform((x1 + 0.005 - leg_w, box.y1 - leg_dy)))
    ...
    def save(self, suffix="", formt=".jpeg"):
        self.fig.savefig('Fig'+suffix+formt)
        plt.clf()
        plt.cla()
        plt.close()
        gc.collect()
